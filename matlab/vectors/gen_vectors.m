...
for cidx = 1:numel(cases)
    ...
    local_write_memh(fullfile(case_dir, 'input_scalar.memh'), round(c.signal(:) .* 2^spec.fixed_point.input_frac_bits), spec.fixed_point.input_width);
...
cases = { ...
    ...
    struct('name', 'random', 'note', 'Uniform random full-scale', 'signal', 2 * rand(nrand, 1) - 1), ...
...
cases{8}.signal = cases{8}.signal(1:nrand);
end
